load("@//prometheus/internal:toolchain.bzl", "build_toolchains", "prometheus_register_toolchains")
...
def _prometheus_repositories_impl(
        ...
        _prometheus_binary_info = _DEFAULT_PROMETHEUS_BINARY_INFO,
        _alertmanager_binary_info = _DEFAULT_ALERTMANAGER_BINARY_INFO):
    ...
    Args:
        ...
        _prometheus_binary_info: pre-built PrometheusBinaryInfo provider function with info on expected version and available binaries
        _alertmanager_binary_info: pre-built AlertmanagerBinaryInfo provider function with info on expected version and available binaries
    """

    prometheus_package_info = PrometheusPackageInfo(
        platforms_info = _platforms_info,
        prometheus_binary_info = _prometheus_binary_info(version = prometheus_version),
        alertmanager_binary_info = _alertmanager_binary_info(version = alertmanager_version),
    )
...
def prometheus_repositories(
        prometheus_version = _DEFAULT_PROMETHEUS_VERSION,
        alertmanager_version = _DEFAULT_ALERTMANAGER_VERSION):
